...
public class fonction_Alice {
	...
	/**
	 * rappel par le class InterfaceAlice pour realiser la fonctionnalite
	 * d'Alice3
	 * 
	 * @param id
	 *            l'identifiant d'utilisateur qu'on s'interesse
	...
	public static ArrayList<String[]> Alice3(String id) throws IOException, JSONException {
		...
		String json_str_answers = HttpRequest.sendGet(
				Requete_Generateur.Fonction_Users() + Requete_Generateur.Fonction_Users_Answers(id),
				"page=1&pagesize=10&order=desc&sort=votes&site=stackoverflow", true);// demande
																						// les
																						// tops
																						// tags
																						// ï¿½
		...
		for (Answer ans : Answers_List) {
			String json_str_questions = HttpRequest.sendGet(
					Requete_Generateur.Fonction_Answers_Questions(ans.getAnswer_id()),
					Requete_Generateur.GET_Parameters(1, 1), true);
			...
			JSON_Map_questions.put("owner",
					User.MaptoC_User((HashMap<String, Object>) JSON_Map_questions.get("owner")));
	...
	public static ArrayList<String[]> Alice1(String id) throws IOException, JSONException {
		...
		String questions = HttpRequest.sendGet("https://api.stackexchange.com/2.2/questions/unanswered", req, true);// requete
																													// des
																													// 7
																													// dernieres
																													// questions
		...
		for (int i = 0; i < 7; i++) {
			titres[i] = Qtitle(questions, i + 1);// on stoque les libellé–Ÿ des
													// questions lï¿½
			// System.out.println("question: ");
			// System.out.println(titres[i]);
			liens[i] = Qlink(questions, i + 1);// on stoque les liens vers les
												// questions lï¿½
			// System.out.println(liens[i]);
			tags[i] = Qtags(questions, i + 1);// on stoque les tags des
												// questions lï¿½
			// System.out.println("tags: ");
			// System.out.println(tags[i]);
			nb_quest[i] = Qnum(questions, i + 1);// on stoque les numeros des
													// questions lï¿½
		...
		questions = HttpRequest.sendGet("https://api.stackexchange.com/2.2/questions/unanswered", req, true);// 7
																												// dernieres
																												// questions
																												// sans
																												// reponse
		...
		for (int i = 0; i < 7 + sup; i++) {
			if (doublon(Qnum(questions, i + 1), nb_quest)) {// permet d'é–¢iter
															// les doublons
															// (question ayant
															// deux des tags)
				sup++;
		...
		questions = HttpRequest.sendGet("https://api.stackexchange.com/2.2/questions/unanswered", req, true);// pareil
																												// ici
		// System.out.println(questions);
		for (int i = 0; i < 6; i++) {
	...
	public static String Qtags(String q, int num) {// cette fonction cherche la
													...
													// dans le string de retour
													// de stack overflow q et la
													// retourne
		int debut = 0;
		int fin = 0;
		for (int i = 0; i < num; i++) {
	...
	public static String Qnum(String q, int num) {// cette fonction cherche le
													// numé‘�e numero de
													// question dans le string q
													// de retour de stack
													// overflowet la retourne
	...
	public static boolean doublon(String nq, String[] qprec) {// vé–žifie que le
																// string nq
																// (numero
																// question) est
																// absent du
																...
																// (questions
																// precedentes).
		for (int i = 0; i < qprec.length; i++) {// retourne true s'il est
												// present et false sinon
			if (qprec[i] == nq) {
				return true;
			}
		}
	 ...
	 * @return tableau contenant les bagdes,le rang des badges et le liens des
	 *         badges
	 * @throws IOException
	 * @throws JSONException
	 */
	public static ArrayList<String[]> Alice2(String id) throws IOException, JSONException {
		ArrayList<String[]> tab = new ArrayList<String[]>();
		String requete = id + "/badges?order=desc&sort=awarded&site=stackoverflow";
		...
		String badges[] = new String[arr.length()];
		String type[] = new String[arr.length()];
		String link[] = new String[arr.length()];

		for (int i = 0; i < arr.length(); i++) {
			badges[i] = arr.getJSONObject(i).getString("name");
			type[i] = arr.getJSONObject(i).getString("rank");
			link[i] = arr.getJSONObject(i).getString("link");
		}
		tab.clear();
		tab.add(badges);
		tab.add(link);
