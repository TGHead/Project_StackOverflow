...
public class fonction_Alice {
	...
	public static ArrayList<String []> Alice3 (String id) throws IOException, JSONException{
		...
		String json_str_answers = HttpRequest.sendGet(Requete_Generateur.Fonction_Users() + Requete_Generateur.Fonction_Users_Answers(id)
														, "page=1&pagesize=10&order=desc&sort=votes&site=stackoverflow", true);//demande les tops tags � stack overflow
	...
	public static ArrayList<String[]> Alice1(String id) throws IOException, JSONException {
		...
		String json_str_top_tags = HttpRequest.sendGet("https://api.stackexchange.com/2.2/users/", req, false);//demande les tops tags � stack overflow
		HashMap<String, Object> JSON_Map_tags = JSON_Converter.jsonToMap(json_str_top_tags);//les 3 lignes suivantes transforme la reponse de stack overflow en string plus facile � utiliser
		...
		for (int i = 0; i < 7; i++) {
			titres[i] = Qtitle(questions, i + 1);//on stoque les libell閟 des questions l�
			// System.out.println("question: ");
			// System.out.println(titres[i]);
			liens[i] = Qlink(questions, i + 1);//on stoque les liens vers les questions l�
			// System.out.println(liens[i]);
			tags[i] = Qtags(questions, i + 1);// on stoque les tags des questions l�
			// System.out.println("tags: ");
			// System.out.println(tags[i]);
			nb_quest[i] = Qnum(questions, i + 1);// on stoque les numeros des questions l�
		...
		for (int i = 0; i < 7 + sup; i++) {
			if (doublon(Qnum(questions, i + 1), nb_quest)) {//permet d'関iter les doublons (question ayant deux des tags)
				sup++;
			} else {
	...
	public static String Qtags(String q, int num) {//cette fonction cherche la num鑝e liste de tags dans le string de retour de stack overflow q et la retourne
		int debut = 0;
		int fin = 0;
		for (int i = 0; i < num; i++) {
	...
	public static String Qlink(String q, int num) {//cette fonction cherche le num鑝e lien dans le string q  de retour de stack overflowet la retourne
		int debut = 0;
		int fin = 0;
		for (int i = 0; i < num; i++) {
	...
	public static String Qtitle(String q, int num) {//cette fonction cherche le num鑝e libell� de question dans le string q  de retour de stack overflowet la retourne
		int debut = 0;
		int fin = 0;
		for (int i = 0; i < num; i++) {
	...
	public static String Qnum(String q, int num) {//cette fonction cherche le num鑝e numero de question dans le string q  de retour de stack overflowet la retourne
		int debut = 0;
		int fin = 0;
		for (int i = 0; i < num; i++) {
	...
	public static boolean doublon(String nq, String[] qprec) {//v閞ifie que le string nq (numero question) est absent du tableau de string qprec (questions precedentes).
